...
def update():
    ...
    try:
        r1 = requests.get("https://ghproxy.com/https://raw.githubusercontent.com/jiankujidu/chuantou/main/2.1/chuantou8000.py").text
        r2 = re.findall(re.compile("version = \d.\d"), r1)[0].split("=")[1].strip()
        if float(r2) > version:
            print("发现新版本：" + r2)
            print("正在自动更新脚本...")
            os.system("kill -9 `ps -ef | grep 'ngrok8000.py' | grep -v 'grep' | awk '{print $1}'`")
            os.system("rm -f ngrok8000.py")
            os.system("ql raw https://ghproxy.com/https://raw.githubusercontent.com/jiankujidu/chuantou/main/2.1/chuantou8000.py &")
    except:
        pass
...
# 下载Ngrok主程序
def download_ngrok():
    if not os.path.exists("ngrok8000.py"):
        res = requests.get("https://ghproxy.com/https://raw.githubusercontent.com/jiankujidu/chuantou/main/2.1/ngrok8000.py")
        with open("ngrok8000.py", "wb") as f:
            f.write(res.content)
    start_nwct()
...
def start_nwct():
    ...
    for i in range(count):
        ...
        if not process_daemon(qlurl):
            os.system("kill -9 `ps -ef | grep 'ngrok8000.py' | grep -v 'grep' | awk '{print $1}'`")
            sleep(2)
            os.system("python3 ngrok8000.py %s %s %s %s &" %(servers[i]["server"], servers[i]["port"], subdomain, qlhttp_auth))
            print("启动中...")
            sleep(5)
...
def get_server():
    try:
        res = requests.get("https://ghproxy.com/https://raw.githubusercontent.com/jiankujidu/chuantou/main/2.1/server.fd").text
...
if __name__ == '__main__':
    ...
    version = 2.1
    try:
        subdomain = os.environ['qlsubdomain8000']
    except:
        subdomain = ""
    try:
    ...
    if len(subdomain) < 1:
        print("请新增变量qlsubdomain8000指定域名前缀！")
    else:
        if other_character(subdomain):
            ...
            print("变量qlsubdomain8000仅支持英文数字组合！")